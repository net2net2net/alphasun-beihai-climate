# AlphaSun 北海极端气候全景系统 · 安卓 APP

把桌面 Node 版改造成可在安卓手机**独立运行**的 APP（Capacitor 纯前端封装）。

> 路线说明：选择「纯前端」而非「内嵌 Node 后端」，是因为中央气象台卫星/雷达图等产品图依赖服务端代理跨域、且 Node 后端无法在 WebView 中直接运行。纯前端方案用浏览器直连各**权威数据源**，APK 仅约 10 MB、可正常上架。

## 与桌面版的区别

| 能力 | 桌面 Node 版 | 安卓 APP 版 |
|------|------------|-----------|
...
| 卫星/雷达产品图（nmc） | ✅ 服务端代理 | ⚠️ 手机端降级（点 ↗ 看官方原图） |
...
```bash
cd app-android
npm install              # 安装 @capacitor/core / @capacitor/android / @capacitor/cli
...
- **白屏**：Android Studio → Logcat 看 WebView 报错。最多见是 Leaflet/Chart.js 的 CDN（unpkg/jsdelivr）被网络限制。解决：把这两个库下载到 `www/vendor/` 并改 `index.html` 的引用为本地路径。
- **地图空白**：高德底图需联网；确认手机有网。
...
├── tsconfig.json
├── .gitignore            # 排除 node_modules/ android/ *.apk
├── README.md             # 本文件
└── www/                  # 前端（已改造为纯前端）
